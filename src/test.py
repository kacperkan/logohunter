...
from utils import load_extractor_model, load_features, model_flavor_from_name
...
def test(filename):
    ...
    yolo = YOLO(
        **{
            "model_path": "keras_yolo3/yolo_weights_logos.h5",
    ...
    test_dir = os.path.join(
        os.path.dirname(__file__), os.path.pardir, "data/test"
    )
    ...
    input_paths = [
        ...
        "test_champions.jpg",
        "test_duff.jpg",
        "test_underarmour.jpg",
        "test_golden_state.jpg",
    ]
    input_labels = [s.split("test_")[-1].split(".")[0] for s in input_paths]
    input_paths = [
        os.path.join(test_dir, "test_brands/", p) for p in input_paths
    ]
    ...
    images = [
        p
        for p in os.listdir(os.path.join(test_dir, "sample_in/"))
        if p.endswith(".jpg")
    ]
    ...
    for iax in range(2):
        for i in range(len(times_list[0])):
            axes[iax].scatter(
                [candidate_len_list, img_size_list][iax],
                np.array(times_list)[:, i],
            )

        axes[iax].legend(
            ["read img", "get box", "get features", "match", "draw", "save"]
        )
        axes[iax].set(
            xlabel=["number of candidates", "image size"][iax],
            ylabel="Time [sec]",
        )
