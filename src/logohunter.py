...
if __name__ == "__main__":
    ...
    if FLAGS.image:
        ...
        if FLAGS.input_brands == "input":
            print(
                "Input logos to search for in images: (file-by-file or entire directory)"
            )
            ...
            print(
                "Loading input brands from text file: reading "
                + FLAGS.input_brands
            )
            if FLAGS.input_brands.endswith(".txt"):
                with open(FLAGS.input_brands, "r") as file:
                    FLAGS.input_brands = [
                        os.path.abspath(f) for f in file.read().splitlines()
                    ]
            ...
            FLAGS.input_brands = [
                os.path.abspath(os.path.join(FLAGS.input_brands, f))
                for f in os.listdir(FLAGS.input_brands)
        ...
        if FLAGS.input_images.endswith(".txt"):
            ...
            with open(FLAGS.input_images, "r") as file:
                file_list = [
                    line.split(" ")[0] for line in file.read().splitlines()
                ]
            FLAGS.input_images = [os.path.abspath(f) for f in file_list]

        elif FLAGS.input_images == "input":
            print(
                "Input images to be scanned for logos: (file-by-file or entire directory)"
            )
            FLAGS.input_images = parse_input()

        elif os.path.isdir(FLAGS.input_images):
            FLAGS.input_images = [
                os.path.abspath(os.path.join(FLAGS.input_images, f))
                for f in os.listdir(FLAGS.input_images)
                if f.endswith((".jpg", ".png"))
            ]
        ...
        print(
            "Found {} input brands: {}...".format(
                len(FLAGS.input_brands),
                [os.path.basename(f) for f in FLAGS.input_brands[:5]],
            )
        )
        print(
            "Found {} input images: {}...".format(
                len(FLAGS.input_images),
                [os.path.basename(f) for f in FLAGS.input_images[:5]],
            )
        )
    ...
    # elif FLAGS.video:
        ...
        print(
            "Must specify either --image or --video.  See usage with --help."
        )
