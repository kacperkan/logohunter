...
def extract_litw_logos(filename, new_path=""):
    ...
    if new_path != "":
        img_list_lbl = [
            path.replace("/home/ubuntu/logohunter", new_path)
            for path in img_list_lbl
        ]
...
if __name__ == "__main__":
    ...
    model, preprocess_input, input_shape = utils.load_extractor_model(
        "InceptionV3", flavor=0
    )
    ...
    print(
        "Processed {} logos, transformed into feature vectors".format(
            len(features)
        )
    )
    ...
    utils.save_features(
        "inception_logo_features.hdf5", features, brand_map, input_shape
    )
    ...
    for i_layer in range(4):
        ...
        features = utils.features_from_image(
            all_logos, model_out, new_preprocess
        )

        extra = "_trunc{}".format(i_layer) if i_layer > 0 else ""
        utils.save_features(
            "inception_logo_features_200{}.hdf5".format(extra),
            features,
            brand_map,
            input_shape,
        )
    ...
    for n in [224, 128, 64]:
        ...
        utils.save_features(
            "vgg16_logo_features_{}.hdf5".format(n),
            features,
            brand_map,
            input_shape,
        )
