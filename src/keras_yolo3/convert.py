...
def _main(args):
    ...
    if (major * 10 + minor) >= 2 and major < 1000 and minor < 1000:
        ...
        seen = np.ndarray(
            shape=(1,), dtype="int32", buffer=weights_file.read(4)
        )
    ...
    for section in cfg_parser.sections():
        ...
        if section.startswith("convolutional"):
            ...
            if batch_normalize:
                conv_layer = (BatchNormalization(weights=bn_weight_list))(
                    conv_layer
                )
    ...
    if len(out_index) == 0:
        out_index.append(len(all_layers) - 1)
    model = Model(
        inputs=input_layer, outputs=[all_layers[i] for i in out_index]
    )
    print(model.summary())
