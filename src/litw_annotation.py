...
def main(args):
    path_to_images, path_to_classes, path_to_out = (
        args.img_path,
        args.classes_names,
        args.out_name,
    )
    train_test_split, split_class_or_file, flag_closed = (
        args.train_test_split,
        args.split_class_or_file,
        args.closedset,
    )
    ...
    for folder in os.listdir(path_to_images):
        ...
        for file in os.listdir(folder_path):
            ...
            if (not file.endswith(".xml")) or (
                not os.path.exists(
                    os.path.join(folder_path, filename + ".jpg")
                )
            ):
                continue
            ...
            list_file.write(
                os.path.normpath(os.path.join(folder_path, filename + ".jpg"))
            )

            for cls_id, bboxes in convert_annotation(
                os.path.join(folder_path, file), classes
            ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-img_path",
        type=str,
        default="../data/LogosInTheWild-v2/data_cleaned/voc_format",
