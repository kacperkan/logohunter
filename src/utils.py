...
def model_flavor_from_name(path):
    ...
    if filename.startswith("inception"):
        ...
        )  # vgg16_logo_features_NNN.hdf5
        flavor = [224, 128, 64].index(length)
    else:
        raise Exception(
            f"Model not recognized as InceptionV3 or VGG16 from filename: {path}"
        )

    if not os.path.exists(path):
        print(
            f"Features not found on local disk! Downloading from AWS S3 bucket, logohunters3.s3-us-west-2.amazonaws.com/{filename} \n"
        )
...
def save_features(filename, features, brand_map, input_shape):
    ...
    print(
        "Saving {} features into {}... ".format(features.shape, filename),
        end="",
    )
    ...
    for box_list, label, color in zip(box_list_list, label_list, color_list):
        ...
        for box in box_list:
            ...
            for i in range(thickness):
                draw.rectangle(
                    [xmin + i, ymin + i, xmax - i, ymax - i], outline=color
                )
            draw.rectangle(
