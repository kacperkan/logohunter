...
def main(dir_litw):
    ...
    for folder in sorted(os.listdir(dir_litw), key=str.casefold):
        ...
        print(
            ...
            "Downloading images in folder {}...".format(folder),
            end="",
        )

        with open(
            os.path.join(dir_litw, folder, "urls.txt"), "r", errors="ignore"
        ) as txtfile:
            start = time.time()

            img_ids, urls = zip(
                *[line.split("\t") for line in txtfile.readlines()]
            )
            filepaths = [
                os.path.join(dir_litw, folder, "img{}.jpg".format(img_id))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.pardir, "data_litw", "LogosInTheWild-v2", "data"
        ),
